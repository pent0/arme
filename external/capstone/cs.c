...
#ifdef CAPSTONE_HAS_ARM
	static_cast<cs_mode>(~(CS_MODE_LITTLE_ENDIAN | CS_MODE_ARM | CS_MODE_V8 | CS_MODE_MCLASS
...
cs_err CAPSTONE_API cs_open(cs_arch arch, cs_mode mode, csh *handle)
{
	...
	if (arch < CS_ARCH_MAX && cs_arch_init[arch]) {
		...
		ud = static_cast<cs_struct*>(cs_mem_calloc(1, sizeof(*ud)));
		if (!ud) {
			// memory insufficient
			return CS_ERR_MEM;
		}
		...
		case CS_OPT_MNEMONIC:
			...
			if (opt->id) {
				if (opt->mnemonic) {
					...
					if (!tmp) {
						tmp = reinterpret_cast<insn_mnem*>(cs_mem_malloc(sizeof(*tmp)));
						tmp->insn.id = opt->id;
						(void)strncpy(tmp->insn.mnemonic, opt->mnemonic, sizeof(tmp->insn.mnemonic) - 1);
						tmp->insn.mnemonic[sizeof(tmp->insn.mnemonic) - 1] = '\0';
...
size_t CAPSTONE_API cs_disasm(csh ud, const uint8_t *buffer, size_t size, uint64_t offset, size_t count, cs_insn **insn)
{
	...
	insn_cache = static_cast<cs_insn*>(total);
	...
	while (size > 0) {
		...
		if (handle->detail) {
			...
			insn_cache->detail = reinterpret_cast<cs_detail*>(cs_mem_malloc(sizeof(cs_detail)));
		} else {
	...
	*insn = reinterpret_cast<cs_insn*>(total);
...
cs_insn * CAPSTONE_API cs_malloc(csh ud)
{
	...
	insn = reinterpret_cast<cs_insn*>(cs_mem_malloc(sizeof(cs_insn)));
	if (!insn) {
		// insufficient memory
		handle->errnum = CS_ERR_MEM;
		return NULL;
		...
		if (handle->detail) {
			// allocate memory for @detail pointer
			insn->detail = reinterpret_cast<cs_detail*>(cs_mem_malloc(sizeof(cs_detail)));
