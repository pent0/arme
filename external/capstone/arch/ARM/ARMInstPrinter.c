...
void ARM_printInst(MCInst *MI, SStream *O, void *Info)
{
	...
	switch(Opcode) {
		...
		case ARM_MOVsr: {
							...
							SStream_concat0(O, ARM_AM_getShiftOpcStr(static_cast<ARM_AM_ShiftOpc>(opc)));
							switch(opc) {
								default:
									break;
								case ARM_AM_asr:
		...
		if (MI->csh->detail) {
			if (MI->csh->doing_mem) {
				if (MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].mem.base == ARM_REG_INVALID)
					MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].mem.base = 
                        static_cast<arm_reg>(Reg);
				else
					MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].mem.index = 
                        static_cast<arm_reg>(Reg);
			} else {
#ifndef CAPSTONE_DIET
				uint8_t access;
...
static void printSORegRegOperand(MCInst *MI, unsigned OpNum, SStream *O)
{
	...
	if (MI->csh->detail) {
		...
		MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].shift.type =
            static_cast<arm_shifter>((MCOperand_getImm(MO3) & 7) + ARM_SFT_ASR_REG - 1);
		MI->flat_insn->detail->arm.op_count++;
	}
...
static void printSORegImmOperand(MCInst *MI, unsigned OpNum, SStream *O)
{
	...
	if (MI->csh->detail) {
		...
		MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].access = CS_AC_READ;
		MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].shift.type = 
             static_cast<arm_shifter>(MCOperand_getImm(MO2) & 7);
...
static void printAM2PreOrOffsetIndexOp(MCInst *MI, unsigned Op, SStream *O)
{
	...
	if (MI->csh->detail) {
		MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].mem.base = 
            static_cast<arm_reg>(MCOperand_getReg(MO1));
	}
	...
	if (MI->csh->detail) {
		MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].mem.index = 
            static_cast<arm_reg>(MCOperand_getReg(MO2));
		MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].subtracted = subtracted == ARM_AM_sub;
	}
...
static void printCPSIMod(MCInst *MI, unsigned OpNum, SStream *O)
{
	...
	if (MI->csh->detail) {
		MI->flat_insn->detail->arm.cps_mode = 
            static_cast<arm_cpsmode_type>(mode);
	}
}
...
static void printCPSIFlag(MCInst *MI, unsigned OpNum, SStream *O)
{
	...
	if (MI->csh->detail) {
		MI->flat_insn->detail->arm.cps_flag = 
            static_cast<arm_cpsflag_type>(IFlags);
	}
}
...
static void printMSRMaskOperand(MCInst *MI, unsigned OpNum, SStream *O)
{
	...
	if (SpecRegRBit) {
		...
		if (Mask) {
			...
			ARM_addSysReg(MI, static_cast<arm_sysreg>(reg));
		}
	} else {
...
static void printBankedRegOperand(MCInst *MI, unsigned OpNum, SStream *O)
{
	...
	arm_sysreg RegIds[] = {
		...
		ARM_SYSREG_LR_USR, static_cast<arm_sysreg>(0), ARM_SYSREG_R8_FIQ, ARM_SYSREG_R9_FIQ,
		ARM_SYSREG_R10_FIQ, ARM_SYSREG_R11_FIQ, ARM_SYSREG_R12_FIQ,
		ARM_SYSREG_SP_FIQ, ARM_SYSREG_LR_FIQ, static_cast<arm_sysreg>(0), ARM_SYSREG_LR_IRQ,
		ARM_SYSREG_SP_IRQ, ARM_SYSREG_LR_SVC, ARM_SYSREG_SP_SVC,
		ARM_SYSREG_LR_ABT, ARM_SYSREG_SP_ABT, ARM_SYSREG_LR_UND,
		ARM_SYSREG_SP_UND, static_cast<arm_sysreg>(0), static_cast<arm_sysreg>(0), static_cast<arm_sysreg>(0), static_cast<arm_sysreg>(0), ARM_SYSREG_LR_MON, ARM_SYSREG_SP_MON,
...
static void printPredicateOperand(MCInst *MI, unsigned OpNum, SStream *O)
{
	...
	if ((unsigned)CC == 15) {
		...
		if (MI->csh->detail)
			MI->flat_insn->detail->arm.cc = static_cast<arm_cc>(CC + 1);
	}
}
